...
mvee <- function(xy=NULL, tolerance = 0.005, plotme = FALSE, max.iter = 500, shiftxy = TRUE, no.ellipse.val=1, checks=TRUE, colour="blue", lwd=1) {
    ...
    if (plotme) {
      ...
      #       main="minimum volume enclosing ellipsoid", xlab=names(xy)[1], ylab=names(xy)[2])
        points(elp.plot.xs, elp.plot.ys, type = "l", lty="dotted", col=colour, asp=1, lwd=lwd)
